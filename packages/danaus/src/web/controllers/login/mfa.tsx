...
import { verifyMfaLoginForm, type AuthFactor } from './lib/forms.ts';

export default {
	middleware: [forms({ verifyMfaLoginForm })],
	actions: {
		index({ url }) {
			const { accountManager } = getAppContext();

			const redirectUrl = url.searchParams.get('redirect');
			const challenge = url.searchParams.get('token');
		...
		totp({ url }) {
			...
			return render(
				<BaseForm factor="totp" challenge={challenge} redirectUrl={redirectUrl}>
		...
		webauthn() {
			redirect(routes.login.show.href());
		},
		recovery({ url }) {
			const { accountManager } = getAppContext();
			...
			return render(
				<BaseForm factor="recovery" challenge={challenge} redirectUrl={redirectUrl}>
					<div class="flex flex-col gap-2">
						<h1 class="text-base-500 font-semibold">Two-factor authentication</h1>
						<p class="text-base-300 text-neutral-foreground-3">
